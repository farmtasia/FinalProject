...
public class Storekeeper : BaseNPC
{
    ...
    protected override void InitializeNPCData(int npcID)
    {
        ...
        // 상점주인에 상호작용 이벤트 추가
        InteractionEvent storeKeeperInteraction = gameObject.GetComponent<InteractionEvent>();
        if (storeKeeperInteraction == null)
        {
            storeKeeperInteraction = gameObject.AddComponent<InteractionEvent>();
        }
        //storeKeeperInteraction.SetEventInfo(npcID, CurrentEventID); // 상점주인에 대한 이벤트 ID 설정
    }
