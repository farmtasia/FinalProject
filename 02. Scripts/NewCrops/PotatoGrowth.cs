...
public class PotatoGrowth : MonoBehaviour, IInteractable
{
    public Sprite[] cropStages; // 작물 단계별 이미지 배열
    private int currentStage = 0; // 현재 단계
    private float growthTime = 10800f; // 감자의 성장시간. (252000f)
    private float wateringTime; // 물뿌리개로 물 준 시간
    private int wateringDay; // 물을 준 날짜
    private SpriteRenderer growthPotato; // 작물(감자) 스프라이트 렌더러
    private bool canGrowth = false; // 성장 가능 여부

    public GameObject getPotato; // 수확물 오브젝트(감자)
    ...
    private void Awake()
    {
        growthPotato = transform.GetChild(0).GetComponent<SpriteRenderer>(); // 자식오브젝트 중 0번째를 가져온다
    }
    ...
    void UpdateCropStage() // 현재 작물 이미지 업데이트
    {
        ...
        {
            growthPotato.sprite = cropStages[currentStage - 1];
        }
    }
