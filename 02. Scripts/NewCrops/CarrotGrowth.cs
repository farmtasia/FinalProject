...
public class CarrotGrowth : MonoBehaviour, IInteractable
{
    public Sprite[] cropStages; // 작물 단계별 이미지 배열
    private int currentStage = 0; // 현재 단계
    private float growthTime = 7200f; // 당근의 성장시간. (108000f)
    private float wateringTime; // 물뿌리개로 물 준 시간
    private int wateringDay; // 물을 준 날짜
    private SpriteRenderer growthBeet; // 작물(당근) 스프라이트 렌더러
    private bool canGrowth = false; // 성장 가능 여부

    public GameObject getCarrot; // 수확물 오브젝트(당근)
